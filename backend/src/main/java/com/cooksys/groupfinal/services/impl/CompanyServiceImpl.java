...
//		filteredUsers.removeIf(user -> !user.isActive());
	...
	@Override
	public AnnouncementDto addAnnouncement(Long id, AnnouncementRequestDto announcementRequestDto) {
		if(announcementRequestDto == null || announcementRequestDto.getMessage() == null || announcementRequestDto.getTitle() == null) {
			throw new BadRequestException("Please provide a valid announcement in the request body");
		}
		Optional<Company> companyOptional = companyRepository.findById(id);
		if(companyOptional.isEmpty()){
			throw new BadRequestException("Company id does not exist");
		}
		Optional<User> userOptional = userRepository.findById(announcementRequestDto.getUserId());
		if(userOptional.isEmpty()){
			throw new BadRequestException("User id does not exist");
		}
		Announcement announcement = new Announcement();
		announcement.setCompany(companyOptional.get());
		announcement.setAuthor(userOptional.get());
	...
	@Override
	public Set<ProjectDto> getActiveAndInactiveProjects(Long companyId, Long teamId) {
		Company company = findCompany(companyId);
		Team team = findTeam(teamId);
		if (!company.getTeams().contains(team)) {
			throw new NotFoundException("A team with id " + teamId + " does not exist at company with id " + companyId + ".");
